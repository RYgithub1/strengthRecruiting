...
（ストレングス リクルーティング！： 強み採用のプラットフォームアプリケーション）
...
- ヘッダーの、人型アイコンからサインアップ、矢印アイコンからログイン可能です。
...
- (例えログイン済みだとしても、)ログインユーザー自身が投稿したプロフィールではない場合、編集・削除ボタンやスカウト内容は表示されません（他社/他者には編集や閲覧権限がありません）。
...
- （人は複数の強みを持つ可能性があるため、)1ユーザーアカウントにつき、複数のプロフィール作成が可能な設計としています。
